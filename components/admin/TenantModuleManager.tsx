...
export default function TenantModuleManager({ tenant }: Props) {
  ...
  const handleToggle = (moduleKey: string, currentActive: boolean) => {
    ...
    setOptimisticModules((prev) => {
      ...
      if (existing) {
        return prev.map((m) =>
          m.moduleKey === moduleKey ? { ...m, isActive: !currentActive } : m
        );
      }
  ...
  const handleActivateAll = () => {
    ...
    startTransition(async () => {
      try {
        await activateAllModules(tenant.id);
      } catch {
        setOptimisticModules(tenant.modules);
      }
    });
  ...
  const handleDeactivateAll = () => {
    ...
    startTransition(async () => {
      try {
        await deactivateAllModules(tenant.id);
      } catch {
        setOptimisticModules(tenant.modules);
      }
    });
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden transition-all">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <span
                className={`text-[10px] font-black uppercase px-2 py-0.5 rounded ${statusColor}`}
              >
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm font-bold text-neutral-500">
            {activeCount}/{ALL_MODULE_KEYS.length} modulos
          </span>
      ...
      {/* Panel expandible de modulos */}
      {isOpen && (
        <div className="border-t border-neutral-200 dark:border-neutral-800 p-5 animate-in fade-in slide-in-from-top-2 duration-200">
          {/* Acciones masivas */}
            ...
            <div className="mt-5 pt-4 border-t border-neutral-100 dark:border-neutral-800">
              ...
              <div className="flex flex-wrap gap-2">
                {tenant.users.map((user) => (
                  <div
                    key={user.email}
                    className="flex items-center gap-2 bg-neutral-50 dark:bg-neutral-800 px-3 py-1.5 rounded-lg"
                  >
