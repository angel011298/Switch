...
export default function DashboardHub() {
  ...
  const resumen = useMemo(() => {
    const totalIngresos = datos.ingresos.reduce(
      (acc, curr) => acc + (Number(curr.total) || 0),
      0
    );
    const totalGastos = datos.gastos.reduce(
      (acc, curr) => acc + (Number(curr.total) || 0),
      0
    );
    return {
      ...
      pendientesCobro: datos.ingresos.filter((i) => i.estatus === 'Pendiente')
        .length,
    ...
    <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <header>
        <h1 className="text-4xl md:text-5xl font-black text-neutral-950 dark:text-white tracking-tighter leading-tight">
          Un solo movimiento.
          <br />
          <span className="text-emerald-600 italic">Toda tu operacion.</span>
        </h1>
        <p className="mt-4 text-base text-neutral-500 dark:text-neutral-400 font-medium max-w-2xl">
          Centro de mando modular donde controlas cada rincon de tu empresa con
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPICard
          ...
          icon={Wallet}
          accent="amber"
          linkHref="/finanzas/cobranza"
          linkLabel="Ver cobranza"
        />
        <KPICard
          label="Plantilla Activa"
          value={String(resumen.plantilla)}
          icon={Users}
          accent="blue"
        />
    ...
    <div className="bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-neutral-200 dark:border-neutral-800">
      <div className="flex justify-between items-start mb-3">
        ...
        {linkHref ? (
          <Link
            href={linkHref}
            className={`text-[10px] font-black uppercase ${a.text} hover:underline`}
          >
            {linkLabel}
          </Link>
        ) : (
          <span
            className={`text-[10px] font-black uppercase ${a.text}`}
          >
            {label}
          </span>
        )}
